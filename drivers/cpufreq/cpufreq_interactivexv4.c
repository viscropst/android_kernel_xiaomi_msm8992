/*
 * drivers/cpufreq/cpufreq_interactivexv4.c
 *
...
#include <trace/events/cpufreq_interactivexv4.h>

extern bool mdss_screen_on;

struct cpufreq_interactivexv4_policyinfo {
	...
	struct cpufreq_interactivexv4_tunables *cached_tunables;
	unsigned long *cpu_busy_times;
};

/* Protected by per-policy load_lock */
struct cpufreq_interactivexv4_cpuinfo {
	u64 time_in_idle;
...
static DEFINE_PER_CPU(struct cpufreq_interactivexv4_policyinfo *, polinfo);
static DEFINE_PER_CPU(struct cpufreq_interactivexv4_cpuinfo, cpuinfo);
...
struct cpufreq_interactivexv4_tunables {
	int usage_count;
...
static struct cpufreq_interactivexv4_tunables *common_tunables;
static struct cpufreq_interactivexv4_tunables *cached_common_tunables;

static struct attribute_group *get_sysfs_attr(void);

/* Round to starting jiffy of next evaluation window */
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_interactivexv4_tunables *tunables)
{
...
static void cpufreq_interactivexv4_timer_resched(unsigned long cpu,
					      bool slack_only)
{
	struct cpufreq_interactivexv4_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_interactivexv4_cpuinfo *pcpu;
	struct cpufreq_interactivexv4_tunables *tunables =
 ...
 * this function.
 */
static void cpufreq_interactivexv4_timer_start(
	struct cpufreq_interactivexv4_tunables *tunables, int cpu)
{
	struct cpufreq_interactivexv4_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_interactivexv4_cpuinfo *pcpu;
...
static unsigned int freq_to_targetload(
	struct cpufreq_interactivexv4_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_interactivexv4_policyinfo *pcpu,
		unsigned int loadadjfreq)
{
...
{
	struct cpufreq_interactivexv4_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_interactivexv4_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	struct cpufreq_interactivexv4_tunables *tunables =
		ppol->policy->governor_data;
	u64 now;
	u64 now_idle;
...
static void cpufreq_interactivexv4_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_interactivexv4_policyinfo *ppol = per_cpu(polinfo, data);
	struct cpufreq_interactivexv4_tunables *tunables =
		ppol->policy->governor_data;
	struct cpufreq_interactivexv4_cpuinfo *pcpu;
	...
	for_each_cpu(i, ppol->policy->cpus) {
		...
		trace_cpufreq_interactivexv4_cpuload(i, tmploadadjfreq /
						  ppol->target_freq);
	...
	if (cpu_load <= MAX_LOCAL_LOAD &&
		...
		trace_cpufreq_interactivexv4_notyet(
	...
	if (new_freq < ppol->target_freq &&
		...
		trace_cpufreq_interactivexv4_notyet(max_cpu, cpu_load,
	...
	if (ppol->target_freq == new_freq) {
		trace_cpufreq_interactivexv4_already(
			max_cpu, cpu_load, ppol->target_freq,
			ppol->policy->cur, new_freq);
		spin_unlock_irqrestore(&ppol->target_freq_lock, flags);
		goto rearm;
	...
	trace_cpufreq_interactivexv4_target(max_cpu, cpu_load, ppol->target_freq,
					 ppol->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&ppol->policy_timer))
		cpufreq_interactivexv4_timer_resched(data, false);
...
static int cpufreq_interactivexv4_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	struct cpufreq_interactivexv4_policyinfo *ppol;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (ppol->target_freq != ppol->policy->cur)
				__cpufreq_driver_target(ppol->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_interactivexv4_setspeed(cpu,
						     max_freq,
						     ppol->policy->cur);
			up_read(&ppol->enable_sem);
		}
...
static void cpufreq_interactivexv4_boost(struct cpufreq_interactivexv4_tunables *tunables)
{
	int i;
	int anyboost = 0;
				...
				void *data)
{
	...
	trace_cpufreq_interactivexv4_load_change(cpu);
	del_timer(&ppol->policy_timer);
	del_timer(&ppol->policy_slack_timer);
	cpufreq_interactivexv4_timer(cpu);
...
static int cpufreq_interactivexv4_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_interactivexv4_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_interactivexv4_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_interactivexv4_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_interactivexv4_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_interactivexv4_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_interactivexv4_tunables
...
static ssize_t show_min_sample_time(struct cpufreq_interactivexv4_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_interactivexv4_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_interactivexv4_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_interactivexv4_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	struct cpufreq_interactivexv4_tunables *t;
	int cpu;
...
static ssize_t show_timer_slack(struct cpufreq_interactivexv4_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_interactivexv4_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_interactivexv4_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_interactivexv4_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_interactivexv4_boost("on");
		if (!tunables->boosted)
			cpufreq_interactivexv4_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_interactivexv4_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_interactivexv4_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_interactivexv4_boost("pulse");
	if (!tunables->boosted)
		cpufreq_interactivexv4_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_interactivexv4_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_interactivexv4_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_screen_off_maxfreq(struct cpufreq_interactivexv4_tunables *tunables,
                char *buf)
{
	return sprintf(buf, "%lu\n", screen_off_max);
}

static ssize_t store_screen_off_maxfreq(struct cpufreq_interactivexv4_tunables *tunables,
                const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_interactivexv4_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_interactivexv4_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static int cpufreq_interactivexv4_enable_sched_input(
			struct cpufreq_interactivexv4_tunables *tunables)
{
	int rc = 0, j;
	struct cpufreq_interactivexv4_tunables *t;
...
static int cpufreq_interactivexv4_disable_sched_input(
			struct cpufreq_interactivexv4_tunables *tunables)
{
	mutex_lock(&sched_lock);
			...
			const char *buf, size_t count)
{
	...
	if (val)
		ret = cpufreq_interactivexv4_enable_sched_input(tunables);
	else
		ret = cpufreq_interactivexv4_disable_sched_input(tunables);
...
static struct attribute *interactivexv4_attributes_gov_sys[] = {
...
static struct attribute_group interactivexv4_attr_group_gov_sys = {
	.attrs = interactivexv4_attributes_gov_sys,
	.name = "interactivexv4",
};

/* Per policy governor instance */
static struct attribute *interactivexv4_attributes_gov_pol[] = {
...
static struct attribute_group interactivexv4_attr_group_gov_pol = {
	.attrs = interactivexv4_attributes_gov_pol,
	.name = "interactivexv4",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &interactivexv4_attr_group_gov_pol;
	else
		return &interactivexv4_attr_group_gov_sys;
}

static void cpufreq_interactivexv4_nop_timer(unsigned long data)
{
}

static struct cpufreq_interactivexv4_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_interactivexv4_tunables *tunables;
...
static struct cpufreq_interactivexv4_policyinfo *get_policyinfo(
					struct cpufreq_policy *policy)
{
	struct cpufreq_interactivexv4_policyinfo *ppol =
	...
	ppol->policy_timer.function = cpufreq_interactivexv4_timer;
	init_timer(&ppol->policy_slack_timer);
	ppol->policy_slack_timer.function = cpufreq_interactivexv4_nop_timer;
...
static struct cpufreq_interactivexv4_tunables *get_tunables(
				struct cpufreq_interactivexv4_policyinfo *ppol)
{
	if (have_governor_per_policy())
...
static int cpufreq_governor_interactivexv4(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	struct cpufreq_interactivexv4_policyinfo *ppol;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (tunables->use_sched_load)
			cpufreq_interactivexv4_enable_sched_input(tunables);
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		...
		if (tunables->use_sched_load)
			cpufreq_interactivexv4_disable_sched_input(tunables);
	...
	case CPUFREQ_GOV_START:
		...
		ppol->last_evaluated_jiffy = get_jiffies_64();
		cpufreq_interactivexv4_timer_start(tunables, policy->cpu);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (ppol->governor_enabled) {
			...
			if (policy->min < ppol->min_freq)
				cpufreq_interactivexv4_timer_resched(policy->cpu,
								  true);
			ppol->min_freq = policy->min;
		}
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVEXV4
static
#endif
...
static int __init cpufreq_interactivexv4_init(void)
{
	...
	speedchange_task =
		kthread_create(cpufreq_interactivexv4_speedchange_task, NULL,
			       "cfinteractivexv4");
	...
	return cpufreq_register_governor(&cpufreq_gov_interactivexv4);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERACTIVEXV4
fs_initcall(cpufreq_interactivexv4_init);
#else
module_init(cpufreq_interactivexv4_init);
#endif

static void __exit cpufreq_interactivexv4_exit(void)
{
	int cpu;

	cpufreq_unregister_governor(&cpufreq_gov_interactivexv4);
...
module_exit(cpufreq_interactivexv4_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_interactivexv4' - A cpufreq governor for "
